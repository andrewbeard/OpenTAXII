...
class PlainRenderer:
    def __call__(self, logger, name, event_dict):
        ...
        return "{timestamp} [{logger}] {level}: {event} {pairs}".format(
            timestamp=timestamp,
            logger=logger,
            level=level,
            event=event,
...
def configure_logging(logging_levels, plain=False, stream=sys.stderr):
    renderer = PlainRenderer() if plain else structlog.processors.JSONRenderer()
...
def sync_collections(server, collections, force_deletion=False):
    ...
    for collection in collections:
        ...
        if existing:
            ...
            log.info("sync_collections.updated", name=cobj.name, id=cobj.id)
            updated_counter += 1
        else:
            cobj = manager.create_collection(CollectionEntity(**collection_data))
            manager.set_collection_services(cobj.id, service_ids)
            log.info("sync_collections.created", name=cobj.name, id=cobj.id)
            created_counter += 1
